...
p->setRenderHint(QPainter::Antialiasing, true);
    ...
    // 2) Overlay de estado (rojo si separada, verde si unida o si hay previewSnap)
    ...
    // mismo contorno que el cuerpo (sin las perillas), para que se vea “por encima”
    QRectF body(0, 0, m_size.width(), m_size.height());
    ...
    if (isSelected()) {
        ...
        if (hasLeftKnob)  p->drawEllipse(leftConnector(), 10, 10);
        if (hasRightKnob) p->drawEllipse(rightConnector(), 10, 10);
    }
}
...
void BlockItem::attachRight(BlockItem* child) {
    ...
    for (BlockItem* n = this; n; n = n->m_right) if (n == child) return;

    if (m_right)      { m_right->m_left = nullptr; m_right->update(); }
    if (child->m_left){ child->m_left->m_right = nullptr; child->m_left->update(); }
...
void BlockItem::mouseReleaseEvent(QGraphicsSceneMouseEvent* ev) {
    ...
    // tras conectar/desconectar, actualiza también vecinos para que cambie su color
...
{
    if (!ev->mimeData()->hasFormat("application/x-dnditemdata")) { ev->ignore(); return; }

    QByteArray itemData = ev->mimeData()->data("application/x-dnditemdata");
    QDataStream dataStream(&itemData, QIODevice::ReadOnly);

    QPixmap px; QPoint offset;
    ...
    // it->setPos(ev->scenePos() - QPointF(offset));  // posición soltada
...
BlockItem* BlockItem::findSnapCandidate(qreal maxDistPx) const {
    ...
    for (QGraphicsItem* it : scene()->items(probe)) {
        ...
        if (this->collidesWithItem(blk)) { best = blk; break; }
    }
    return best;
}
